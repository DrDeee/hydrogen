...
function encodeKey$1(roomId, eventType, stateKey) {
...
class RoomStateStore {
    ...
    get(roomId, type, stateKey) {
        const key = encodeKey$1(roomId, type, stateKey);
        return this._roomStateStore.get(key);
    }
    set(roomId, event) {
        const key = encodeKey$1(roomId, event.type, event.state_key);
...
function encodeKey$2(roomId, userId) {
...
class RoomMemberStore {
	...
	get(roomId, userId) {
        return this._roomMembersStore.get(encodeKey$2(roomId, userId));
	}
	async set(member) {
        member.key = encodeKey$2(member.roomId, member.userId);
        return this._roomMembersStore.put(member);
	}
    getAll(roomId) {
        const range = this._roomMembersStore.IDBKeyRange.lowerBound(encodeKey$2(roomId, ""));
...
function encodeKey$3(roomId, fragmentId) {
...
class TimelineFragmentStore {
    ...
    add(fragment) {
        fragment.key = encodeKey$3(fragment.roomId, fragment.id);
    ...
    get(roomId, fragmentId) {
        return this._store.get(encodeKey$3(roomId, fragmentId));
    }
    removeAllForRoom(roomId) {
...
class DeviceIdentityStore {
    ...
    getAllForUserId(userId) {
        const range = this._store.IDBKeyRange.lowerBound(encodeKey$5(userId, ""));
    ...
    get(userId, deviceId) {
        return this._store.get(encodeKey$5(userId, deviceId));
    }
    set(deviceIdentity) {
        deviceIdentity.key = encodeKey$5(deviceIdentity.userId, deviceIdentity.deviceId);
...
function encodeKey$6(senderKey, sessionId) {
...
class OlmSessionStore {
    ...
    async getSessionIds(senderKey) {
        const sessionIds = [];
        const range = this._store.IDBKeyRange.lowerBound(encodeKey$6(senderKey, ""));
        await this._store.iterateKeys(range, key => {
    ...
    get(senderKey, sessionId) {
        return this._store.get(encodeKey$6(senderKey, sessionId));
    }
    set(session) {
        session.key = encodeKey$6(session.senderKey, session.sessionId);
        return this._store.put(session);
    }
    remove(senderKey, sessionId) {
        return this._store.delete(encodeKey$6(senderKey, sessionId));
    }
}

function encodeKey$7(roomId, senderKey, sessionId) {
...
class InboundGroupSessionStore {
    ...
    async has(roomId, senderKey, sessionId) {
        const key = encodeKey$7(roomId, senderKey, sessionId);
        const fetchedKey = await this._store.getKey(key);
        return key === fetchedKey;
    }
    get(roomId, senderKey, sessionId) {
        return this._store.get(encodeKey$7(roomId, senderKey, sessionId));
    }
    set(session) {
        session.key = encodeKey$7(session.roomId, session.senderKey, session.sessionId);
        this._store.put(session);
    }
    removeAllForRoom(roomId) {
        const range = this._store.IDBKeyRange.bound(
            encodeKey$7(roomId, MIN_UNICODE, MIN_UNICODE),
            encodeKey$7(roomId, MAX_UNICODE, MAX_UNICODE)
        );
        this._store.delete(range);
    }
}
...
function encodeKey$8(roomId, sessionId, messageIndex) {
...
class GroupSessionDecryptionStore {
    ...
    get(roomId, sessionId, messageIndex) {
        return this._store.get(encodeKey$8(roomId, sessionId, messageIndex));
    }
    set(roomId, sessionId, messageIndex, decryption) {
        decryption.key = encodeKey$8(roomId, sessionId, messageIndex);
